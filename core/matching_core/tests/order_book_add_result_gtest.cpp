        ...
        for (auto level_it = book.begin(); level_it != book.end(); ++level_it) {
            ...
            const auto order_it =
                std::find_if(queue.begin(), queue.end(),
                             [&](const RefOrder& order) { return order.id == order_id; });
...
void ExpectSameTrade(const matching::Trade& actual,
                     const matching::Trade& expected, std::size_t index) {
...
class ResultHarness {
public:
    explicit ResultHarness(std::size_t capacity = 200000) : actual_(capacity) {}

    void add_limit(std::uint64_t order_id, matching::Side side,
        ...
        apply_result(order_id, actual);
    }

    void add_market(std::uint64_t order_id, matching::Side side,
    ...
    void modify(std::uint64_t order_id, matching::Side side,
                std::int64_t price, std::uint64_t quantity,
                std::uint64_t timestamp) {
        const auto expected =
    ...
    void random_test(std::uint64_t seed = 42, std::size_t ops = 5000) {
        ...
        for (std::size_t i = 0; i < ops; ++i) {
            if (known_orders.empty() || rng() % 4 != 0) {
                ...
                const std::int64_t price = 1000 + static_cast<std::int64_t>(rng() % 100)
                    * (side == matching::Side::Buy ? -1 : 1);
...
private:
    ...
    void apply_result(std::uint64_t order_id, const matching::AddResult& result) {
