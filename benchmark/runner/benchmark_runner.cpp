...
int RunScenario(IBenchScenario& scenario, int argc, char** argv) {
    ...
    // Cap batch_size to the scenario's safe limit (1 for destructive ops).
    ...
    // Warms up caches, branch predictors, and any lazy initialisation so
    // the measured phase sees a steady-state system.
    ...
    // (timed) → Teardown (untimed).  This keeps setup/teardown overhead out of
    ...
    for (std::uint64_t i = 0; i < args.iters; ++i) {
        ...
        if (args.metric == MetricMode::Latency) {
            // Record average ns-per-op for this iteration (batch amortisation).
            const double ns =
            ...
            // Accumulate PMC deltas across iterations for aggregate reporting.
    ...
    if (args.metric == MetricMode::Latency) {
        ...
        // ops_s = theoretical throughput if latency were the bottleneck (1e9 / avg_ns).
        ...
        if (!args.out_csv.empty()) {
            // EnsureCsvHeader is idempotent — only writes on first access.
            EnsureCsvHeader(
                    args.out_csv,
    ...
    // --- PMC output ---
    // Divide by total measured ops to get per-operation averages.
