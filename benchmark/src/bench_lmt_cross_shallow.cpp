...
class LmtCrossShallowScenario : public benchmark_runner::IBenchScenario {
    ...
    // Destructive: consumes orders from the book.
    ...
    void Setup(const benchmark_runner::Args& args, std::uint64_t iter_idx) override {
        ...
        // Cross only the first 3 levels: set price just high enough to touch
        // level 3 (price = 1000 + 3 - 1 = 1002), and quantity to consume 3
        // full levels plus 5 more to test partial level fill.
