...
class DupRejectScenario : public benchmark_runner::IBenchScenario {
    ...
    // Non-destructive: duplicate insertions are rejected without modifying
    // the book, so many RunOp calls can share one Setup.
    ...
    void Setup(const benchmark_runner::Args& args, std::uint64_t iter_idx) override {
        ...
        id_base_ = 500'000ULL;
        // Prefill establishes a set of known-active IDs to collide against.
    ...
    bool RunOp(const benchmark_runner::Args&, std::uint64_t,
        ...
        // Pick a random ID that already exists — the insert must be rejected.
