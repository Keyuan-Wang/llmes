...
class CxlMissScenario : public benchmark_runner::IBenchScenario {
    ...
    // Non-destructive: cancels always miss, so the book state is unchanged
    // after each call.  Many RunOp calls can share one Setup.
    ...
    void Setup(const benchmark_runner::Args& args, std::uint64_t iter_idx) override {
        ...
        // No orders placed in the 9-billion range — guaranteed miss.
    }

    bool RunOp(const benchmark_runner::Args&, std::uint64_t,
                 std::uint64_t, std::uint64_t& ok) override {
        // IDs in the 9-billion range are guaranteed to not exist in the book.
