...
public:
    ...
    void Setup(const benchmark_runner::Args& args, std::uint64_t iter_idx) override {
        // The pool must be large enough for all orders throughout the run:
        // 2× prefilled plus the new ops in this iteration.
        ...
        // Separate RNG for operation-type selection so ID and type are
        ...
        // Prefill asks at prices 1000, 1001, 1002, ...
        ...
        // Prefill bids at prices 999, 998, 997, ... (will not cross with asks,
        // so both sides are independently bookable).
        for (std::uint64_t lvl = 0; lvl < args.levels; ++lvl) {
        ...
        id_counter_ = 1'000'000ULL;               // New orders start here.
        prefilled_begin_ = 100'000ULL;             // Start of prefilled ID range.
        prefilled_count_ = 2 * args.orders;        // Total prefilled orders.
    }
    ...
    bool RunOp(const benchmark_runner::Args& args, std::uint64_t,
        ...
        if (roll < 35) {
            // 35%: Cancel — pick a random ID from the prefilled range.
            // Some of these will hit (order still exists), some will miss
            // (already cancelled in an earlier op).  Both outcomes are valid.
            ...
            // Misses test the modify-as-insert path.
            ...
            // 25%: Limit order at a below-market price (resting, no fill).
            ...
            // 5%: Limit order that crosses the ask spread (partial fill + rest).
            const std::uint64_t oid = id_counter_++;
            ...
            // 5%: Market order — sweeps the best ask levels.
            const std::uint64_t oid = id_counter_++;
...
private:
    ...
    benchmark_runner::SplitMix64 op_rng_{42};     // Independent RNG for op-type selection.
