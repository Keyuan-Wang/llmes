...
/**
	...
	*/
inline void PrefillSellBook(llme::matching::OrderBook& book, std::uint64_t orders,
														std::uint64_t levels, std::uint64_t id_base,
														std::vector<llme::matching::OrderHandle>* handles = nullptr) {
	...
	for (std::uint64_t lvl = 0; lvl < levels; ++lvl) {
		...
		for (std::uint64_t j = 0; j < per_level; ++j) {
			const auto res =
					book.add_limit_order(id, llme::matching::Side::Sell, ask_price, 1, id);
...
/**
	...
	*/
inline void PrefillHftBook(llme::matching::OrderBook& book,
														std::uint64_t orders,
														std::uint64_t levels,
														std::int64_t base_price = 1000,
														std::uint64_t id_base = 100'000'000ULL,
														std::uint64_t seed = 42,
														llme::matching::Side side = llme::matching::Side::Sell,
														std::vector<llme::matching::OrderHandle>* handles = nullptr) {
	SplitMix64 rng(seed);
	std::uint64_t id = id_base;
